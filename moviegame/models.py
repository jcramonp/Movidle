...
class Pelicula(models.Model):
    ...
    actores = models.CharField(max_length=512, blank=True)       # "Actor1, Actor2, Actor3, ..."
    duracion_min = models.PositiveIntegerField("duración (min)", default=0)

    # Datos de OMDb que usaremos en el juego
    imdb_rating = models.DecimalField("IMDb rating", max_digits=3, decimal_places=1, null=True, blank=True)  # 0.0..10.0
    imdb_votes = models.PositiveIntegerField("IMDb votes", null=True, blank=True)  # se usará como “popularidad”
...
class Feedback(models.Model):
    ...
    color_anio = models.CharField(max_length=10, choices=ColorCategoria.choices,
                                  default=ColorCategoria.GRIS)
    flecha_anio = models.CharField(max_length=4, choices=DireccionFlecha.choices,
                                   blank=True, default=DireccionFlecha.NONE)

    # 2) Popularidad por votos (color + flecha) — usamos imdb_votes
    color_popularidad = models.CharField(max_length=10, choices=ColorCategoria.choices,
                                         default=ColorCategoria.GRIS)
    flecha_popularidad = models.CharField(max_length=4, choices=DireccionFlecha.choices,
                                          blank=True, default=DireccionFlecha.NONE)
    ...
    color_duracion = models.CharField(max_length=10, choices=ColorCategoria.choices,
                                      default=ColorCategoria.GRIS)
    flecha_duracion = models.CharField(max_length=4, choices=DireccionFlecha.choices,
                                       blank=True, default=DireccionFlecha.NONE)

    # 5) Director (solo gris/verde)
    color_direccion = models.CharField(max_length=10, choices=ColorCategoria.choices,
                                       default=ColorCategoria.GRIS)
    ...
    color_rating = models.CharField(max_length=10, choices=ColorCategoria.choices,
                                    default=ColorCategoria.GRIS)
